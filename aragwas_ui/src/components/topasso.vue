            ...
            <v-flex xs9 wrap>
                ...
                >
                    ...
                    <template slot="items" scope="props">
                        <td v-if="'snp' in props.item && !('name' in hideFields)">{{ props.item.snp.chr | capitalize }}:{{ props.item.snp.position }}</td> <td v-else>Missing info</td>
                        <td v-if="!('score' in hideFields)" class="text-xs-right">{{ props.item.score | round }}</td>
                        <td v-if="!('phenotype' in hideFields)" class="text-xs-right"><router-link :to="{name: 'phenotypeDetail', params: { id: props.item.study.phenotype.id }}">{{ props.item.study.phenotype.name }}</router-link></td>
                        <td v-if="'snp' in props.item && !('gene' in hideFields)" class="text-xs-right"><router-link :to="{name: 'geneDetail', params: { geneId: props.item.snp.geneName }}">{{ props.item.snp.geneName }}</router-link></td><td v-else class="text-xs-right">Missing info</td>
                        <td v-if="!('maf' in hideFields)" class="text-xs-right">{{ props.item.maf | round }}</td>
                        <td v-if="!('study' in hideFields)" class="text-xs-right"><router-link :to="{name: 'studyDetail', params: { id: props.item.study.id }}">{{ props.item.study.name }}</router-link></td>
                    </template>
                </v-data-table>
...
<script lang="ts">
    ...
    export default class TopAssociationsComponent extends Vue {
        ...
        loadData(pageToLoad): void {
            this.loading = true;
            this.filters = {chr: this.chr, annotation: this.annotation, maf: this.maf, type: this.type};
        ...
        hideHeaders(fields): void {
            for(let i = 0; i<this.headers.length; i++) {
                if (this.headers[i].name in this.hideFields){
